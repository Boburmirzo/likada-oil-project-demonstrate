...
public class AddressRegion implements Serializable{
    ...
    @OneToMany(mappedBy = "addressFactRegion")
    private Set<Requisite> factRequisites;

    @OneToMany(mappedBy = "addressLegalRegion")
    private Set<Requisite> legalRequisites;

    @Override
    public String toString() {
        return "ADDRESS_REGION [id=" + id + ", NAME_FULL=" + nameFull + ", NAME_SHORT=" + nameShort + ", DESCRIPTION=" + description;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof AddressRegion))
            return false;
        AddressRegion other = (AddressRegion) obj;
        if (id == null) {
            if (other.id != null)
                return false;
